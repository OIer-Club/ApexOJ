...
// 读取 config
let configx = JSON.parse(fs.readFileSync(path.join(__dirname + "/config", 'config.json'), {encoding: 'utf8',flag: 'r'}).toString());

var email = {
    config: {
        host: configx.server.smtp.host,
        port: configx.server.smtp.port,
        account: configx.server.smtp.account,
        pwd: configx.server.smtp.pwd
    },
    async send(obj) {
        //connect
